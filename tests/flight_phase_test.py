...
class TestInAir(unittest.TestCase):
    ...
    def test_inair_phase_basic(self):
        vert_spd_data = np.ma.array([0] * 5 + range(0,400,20)+
                                    range(400,-400,-20)+
                                    range(-400,50,20))
        altitude = P('Altitude AAL For Flight Phases', integrate(vert_spd_data, 1, 0, 1.0/60.0))
    ...
    def test_inair_phase_not_fast(self):
        ...
        alt_aal = P('Altitude AAL For Flight Phases', altitude_data)
        fast = []
        air = InAir()
...
class TestApproachAndLanding(unittest.TestCase):
    def test_approach_and_landing_basic(self):
        ...
        app.derive(
            P('Altitude AAL For Flight Phases', alt), 
            lands=phase('[11..20]'), 
...
class TestBouncedLanding(unittest.TestCase):
    def test_bounce_basic(self):
        ...
        bl = BouncedLanding()
        bl.derive(P('Altitude AAL', alt), air)
...
class TestInitialApproach(unittest.TestCase):
    ...
    def test_initial_approach_phase_basic(self):
        ...
        app = InitialApproach()
        alt_aal = P('Altitude AAL For Flight Phases',alt)
    ...
    def test_initial_approach_phase_over_high_ground(self):
        ...
        alt_rad = alt_aal - 600
        app = InitialApproach()
        alt_aal = P('Altitude AAL For Flight Phases',alt_aal)
        app_land = SectionNode('Approach',
...
class TestClimbCruiseDescent(unittest.TestCase):
    ...
    def test_climb_cruise_descent_all_high(self):
        ...
        air = phase('(...5]')
        camel.derive(P('Altitude AAL For Flight Phases',
...
class TestClimbFromBottomOfDescent(unittest.TestCase):
    ...
    def test_descent_to_bottom_of_descent_basic(self):
        ...
        alt = P('Altitude STD', alt_data)

        ccd = ClimbCruiseDescent()
        ccd.derive(P('Altitude For Flight Phases', alt_data))
        toc = TopOfClimb()
        toc.derive(alt, ccd)
        dlc = DescentLowClimb()
        dlc.derive(alt)
...
class TestCruise(unittest.TestCase):
    ...
    def test_cruise_truncated_start(self):
        ...
        alt = P('Altitude STD', alt_data)
        ccd = ClimbCruiseDescent()
        ccd.derive(P('Altitude For Climb Cruise Descent', alt_data),
                   phase('(...)'))
        toc = TopOfClimb()
        toc.derive(alt, ccd)
        tod = TopOfDescent()
...
class TestDescentLowClimb(unittest.TestCase):
    ...
    def test_descent_low_climb_basic(self):
        ...
        alt_aal = P('Altitude AAL For Flight Phases',
                            np.ma.array(testwave))
        #descend = P('Descend For Flight Phases', np.ma.array(dsc))
        #climb = P('Climb For Flight Phases', np.ma.array(clb))
        dlc = DescentLowClimb()
        dlc.derive(alt_aal)
...
class TestFast(unittest.TestCase):
    ...
    def test_fast_phase_basic(self):
        slow_and_fast_data = np.ma.array(range(60, 120, 10) + [120] * 300 + \
                                         range(120, 50, -10))
        ias = P('Airspeed For Flight Phases', slow_and_fast_data,1,0)
        phase_fast = Fast()
...
class TestGrounded(unittest.TestCase):
    ...
    def test_grounded_phase_basic(self):
        airs = phase(
            '[723.5..1001.5], [1026.5..1321.5], [1346.5..1981.5], '
            '[2002.5..2419.5], [7593.5..7930.5], [7956.5..8288.5], '
            '[8313.5..8603.5], [8630.5..10113.5]')
        phase_grounded = Grounded()
        phase_grounded.derive(airs)
        self.assertEqual(phase_grounded, 
            '(...723.5), (1001.5..1026.5), (1321.5..1346.5), '
            '(1981.5..2002.5), (2419.5..7593.5), (7930.5..7956.5), '
            '(8288.5..8313.5), (8603.5..8630.5), (10113.5..)')
...
class TestGoAroundAndClimbout(unittest.TestCase):
    ...
    def test_go_around_and_climbout_phase_basic(self):
        ...
        ga_phase.derive(P('Descend For Flight Phases',down),
                        P('Climb For Flight Phases',up),
                        ga_kti)
...
class TestHolding(unittest.TestCase):
    ...
    def test_rejected_outside_height_range(self):
        ...
        self.assertEqual(hold, '[1000..2700]')
    ...
    def test_hold_detected(self):
        ...
        hold = Holding()
        hold.derive(alt, hdg, lat, lon)
        self.assertEqual(hold, '[570..1289],[1470..2339]')
...
class TestTurning(unittest.TestCase):
    ...
    def test_turning(self):
        ...
        rate_of_turn = P('Rate Of Turn', np.ma.array(rate_of_turn_data))
        turning = Turning()
