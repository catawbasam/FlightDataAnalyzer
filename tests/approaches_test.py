...
class TestApproachSlices(unittest.TestCase):
    def test_approach_slices_basic(self):
    ...
    def test_approach_slices_separate_landing_phase_go_around(self):
        alt = np.ma.array([3500, 2500, 2000, 2500, 3500, 3500])
        land = buildsection('Landing', 5, 6)
...
class TestApproaches(unittest.TestCase):
    ...
    @patch('analysis_engine.approaches.approach_slices')
    def test_derive(self, approach_slices, get_nearest_airport,
                    get_nearest_runway):
        approaches = Approaches()
        approaches._lookup_airport_and_runway = Mock()
        approaches._lookup_airport_and_runway.return_value = [None, None]
        approach_slices.return_value = []
        ...
        approach_slices.return_value = slices
        ...
        self.assertEqual(approaches,
                         [Approach('TOUCH_AND_GO', slice(0, 5)),
                          Approach('GO_AROUND', slice(10, 15)),
                          Approach('LANDING', slice(20, 25))])
        ...
        approaches._lookup_airport_and_runway.assert_has_calls([
            call(_slice=slices[0], appr_hdg=[], appr_lat=[], appr_lon=[],
                 appr_ils_freq=[], precise=False),
            call(_slice=slices[1], appr_hdg=[], appr_lat=[], appr_lon=[],
                 appr_ils_freq=[], precise=False),
            call(_slice=slices[2], appr_hdg=[], appr_lat=[], appr_lon=[],
                 appr_ils_freq=[], precise=False,
        ...
        # lat/lon/hdg:
        approaches.derive(alt_aal, landing, go_arounds, alt_aal, fast, land_hdg,
                          land_lat, land_lon, appr_hdg, appr_lat, appr_lon,
                          land_afr_apt=land_afr_apt_none,
        ...
        self.assertEqual(approaches,
                         [Approach('TOUCH_AND_GO', slice(0, 5)),
                          ...
                          Approach('LANDING', slice(20, 25))])
        approaches._lookup_airport_and_runway.assert_has_calls([
            call(_slice=slices[0], appr_hdg=appr_hdg, appr_lat=appr_lat,
                 appr_lon=appr_lon, appr_ils_freq=[], precise=False),
            call(_slice=slices[1], appr_hdg=appr_hdg, appr_lat=appr_lat,
                 appr_lon=appr_lon, appr_ils_freq=[], precise=False),
            call(_slice=slices[2], appr_hdg=land_hdg, appr_lat=land_lat,
                 appr_lon=land_lon, appr_ils_freq=[], precise=False,
                 land_afr_apt=land_afr_apt_none, land_afr_rwy=land_afr_rwy_none,
                 hint='landing'),
        ])
        approaches._lookup_airport_and_runway.reset_mock()
